...
def check_soft_esc_ego_poll_collapse() -> None:
    """Soft Esc grab: ≤1 vehicle.sensors.poll per tick, PollGPSGE off that cadence.

    engaged stays false. A reused GPS sample is stale, not a new fix.
    The sensor container matches live BeamNGpy: poll() returns None.
    """
    import time
    ...
    from python.sensors.tech import GPS_POLL_PERIOD_S

    assert GPS_POLL_PERIOD_S >= 0.4
    assert SENSOR_POLL_REUSE_S < GPS_POLL_PERIOD_S

    class LiveSensors:
        """BeamNGpy Sensors: poll() returns None; readings live on .items()."""

        def __init__(self, mapping: dict) -> None:
            self._sensors = mapping
            self.n = 0
    ...
    class FakeGPS:
        ...
        def poll(self):
            self.n += 1
            # Each PollGPSGE is a new sample. A coalesced grab must keep time=1.
            return [{
                "time": float(self.n),
                "lon": 8.81 + 0.01 * (self.n - 1),
                "lat": 53.09,
                "x": 10.0,
                "y": 20.0,
            }]
    ...
    class BoomGPS:
        ...
        def poll(self):
            self.n += 1
            raise RuntimeError("PollGPSGE down")
    ...
    rv = (ROOT / "python" / "run_vision.py").read_text(encoding="utf-8")
    assert "backend.poll_vehicle()" in rv
    assert "read_electrics_inputs(vehicle)" in rv
    ...
    for i in range(ticks):
        ...
        assert el.speed_mps == 8.0 and el.steering_input == 0.2, el
        assert data.speed_mps == 8.0 and data.steering_input == 0.2
        assert veh.sensors.n == i + 1, (i, veh.sensors.n)
        assert gps.n == 1, (i, gps.n)
        if i == 0:
            assert data.sensors["gps"] == "ok"
            assert data.gps_time == 1.0 and data.lat == 53.09
            assert data.lon == 8.81
        else:
            assert data.sensors["gps"] == "stale"
            assert data.gps_time == 1.0 and data.lon == 8.81
            assert data.lat == 53.09

    assert veh.sensors.n == ticks
    assert gps.n == 1
    stale = session.poll()
    assert veh.sensors.n == ticks + 1
    assert gps.n == 1 and stale.sensors["gps"] == "stale"
    # Reused fix still aims from pose, not from a pretend new GPS track.
    veh.state["dir"] = (1.0, 0.0, 0.0)
    turned = session.poll()
    read_electrics_inputs(veh)
    assert turned.sensors["gps"] == "stale"
    assert turned.gps_heading_deg is not None and abs(turned.gps_heading_deg - 90.0) < 1e-6
    assert gps.n == 1 and veh.sensors.n == ticks + 2
    snap = nav_snapshot(turned)
    assert snap["mode"] == "hint" and snap["drive_to_pin"] is False
    assert snap["gps"]["ok"] is True and snap["gps"]["lat"] == 53.09
    assert "not a new fix" in snap["note"]

    # Period elapsed: one PollGPSGE, status ok, new sample time.
    session._gps_mono = time.monotonic() - GPS_POLL_PERIOD_S - 0.01
    fresh = session.poll()
    n_after = veh.sensors.n
    read_electrics_inputs(veh)
    assert gps.n == 2 and fresh.sensors["gps"] == "ok" and fresh.gps_time == 2.0
    assert veh.sensors.n == n_after, "electrics read must not add a sensors.poll"
    assert "not a new fix" not in nav_snapshot(fresh)["note"]

    # A down PollGPSGE does not look fresh and does not retry on the next grab.
    boom = BoomGPS()
    session._gps = boom
    session._gps_mono = time.monotonic() - GPS_POLL_PERIOD_S - 0.01
    failed = session.poll()
    assert boom.n == 1 and failed.sensors["gps"] == "stale" and failed.gps_time == 2.0
    again = session.poll()
    read_electrics_inputs(veh)
    assert boom.n == 1 and again.sensors["gps"] == "stale"

    # Expired snapshot still polls (Engage hold miss path). Soft Esc does not.
    session.poll()
    held = _sensor_snaps.get(id(veh))
    assert held is not None and held.used is False
    held.mono -= 1.0
    before = veh.sensors.n
    read_electrics_inputs(veh)
    assert veh.sensors.n == before + 1

    act = BeamNGPyActuator(veh)
    act.note_engaged(False)
    before_s = veh.sensors.n
    before_g = boom.n
    stopped = act.stop(seq=3, reason="not_engaged")
    assert stopped.applied is False and act.engaged is False
    assert veh.controls == []
    assert veh.sensors.n == before_s and boom.n == before_g

    session._gps = gps
    session.close()
    assert gps.removed is True
    print(
        f"soft esc ego poll: sensors.poll/tick=1 gps.poll={gps.n} "
        f"across {ticks} grabs then one deferred PollGPSGE; engaged=false"
    )
...
def main() -> None:
    ...
    check_poll_mock()
    check_soft_esc_ego_poll_collapse()
