...
3. Wait gate before any camera Hz: `:25252` LISTENING, mod present, vehicle spawned, fresh `gvd_link.json` (`lua_bus` age < 1 s), `buses_same(python_bus, lua_bus)` → `link=ok`. Live `lua_bus` wins over the `GVD_DOCS_DIR` guess. Cache reuse is not a pass. Attach passes `socket_timeout` (10 s). Hello timeout is REFUSE and is not a missing vehicle. While the port is LISTENING, launch stays false.
