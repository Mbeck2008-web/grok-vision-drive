...
9. Alt+G engage. Optional pin: `nav.pin_lat` / `nav.pin_lon` in `config/tech.yaml` (or `GVD_NAV_PIN_LAT` / `GVD_NAV_PIN_LON`) — **hint only**, not a route.
...
3. Wait gate before any camera Hz: `:25252` LISTENING, mod present, vehicle spawned, fresh `gvd_link.json` (`lua_bus` age < 1 s), `buses_same(python_bus, lua_bus)` → `link=ok`. Live `lua_bus` wins over the `GVD_DOCS_DIR` guess. Cache reuse is not a pass. Attach bounds `bng.open()` to `socket_timeout` (10 s in `config/tech.yaml`, or `GVD_TECH_SOCKET_TIMEOUT`) on BeamNGpy 1.35 and 1.36. Hello timeout is REFUSE and is not a missing vehicle. While the port is LISTENING, launch stays false.
