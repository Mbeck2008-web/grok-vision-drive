...
Sim-only. Preferred (Tech): BeamNGpy `set_shift_mode("arcade")` + `control(steering, throttle, brake)` on the player vehicle (`get_current` / `get_player_vehicle_id`). Retail: write `Documents/GVD/gvd_cmd.json`; GELua polls it at 20 Hz and applies it as a secondary Direct Drive wheel + pedals (`input.event` source `gvd`, FILTER_DIRECT, `setAllowedInputSource`) — **no DLL**.
