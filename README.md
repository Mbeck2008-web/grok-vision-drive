...
6. Double-click `play_gvd_tech.bat` (or `PYTHONPATH=. python python/run_vision.py --backend beamngpy --viz`). The bat sets `GVD_BEAMNG=1`, `GVD_TECH_LAUNCH=0`, and the Tech bus. It does **not** start Tech. Do not also set `GVD_TECH_LAUNCH=1` (that double-starts). The bat already runs `--tech-hold` and starts the supervisor only when that preflight returns 0. That preflight process disconnects when it exits. Vision/Hz stays down until the research port is LISTENING, the unpacked mod is present, a vehicle is spawned, and `lua_bus` is fresh (< 1 s) with `buses_same` (`link=ok`). The supervisor opens one BeamNGpy session for its wait gate and reuses it for cameras.
...
3. Wait gate before any camera Hz: `:25252` LISTENING, mod present, vehicle spawned, fresh `gvd_link.json` (`lua_bus` age < 1 s), `buses_same(python_bus, lua_bus)` → `link=ok`. The wait-gate line prints that lua folder (`folder=`), including a stale handshake. Live `lua_bus` wins over the `GVD_DOCS_DIR` guess. Cache reuse is not a pass. Attach bounds `bng.open()` to `socket_timeout` (10 s in `config/tech.yaml`, or `GVD_TECH_SOCKET_TIMEOUT`) on BeamNGpy 1.35 and 1.36, and stops the Hello thread on that deadline. Hello timeout is REFUSE and is not a missing vehicle. While the port is LISTENING, launch stays false. The supervisor reuses the hold's BeamNGpy session for cameras. A connect failure after the hold exits the supervisor (exit 1, same as `--tech-probe`).
...
Live Tech attach is **UNPROVEN**. Unique-frame Hz ≥ 10 is **not** claimed here. Details: [Dev (BeamNG.tech)](#dev-beamngtech). Soft Esc parked.
...
- the modular supervisor vetoes an E2E/shadow policy (`--policy e2e|shadow`: low `lane_conf` / `path_conf`, disagreement, AEB). An untrained E2E (`e2e_backend=stub`, no `models/e2e_current.onnx`) is `veto:e2e_stub`: brake hold, stay engaged, HUD **HOLD**. Shadow still applies the modular command and does not disengage on that stub. While engaged, a measured loop or camera rate under `min_accept_hz` (6) disengages on every policy once the engage grace in `config/control.yaml` has ended. During that grace a transient dip down to ~5 Hz stays engaged. The default `modular` policy disengages via Alt+G, override, supervisor exit, the dead-man, and that rate gate.
...
Sim-only. Preferred (Tech): BeamNGpy `set_shift_mode("realistic_automatic")` + `control(steering, throttle, brake, parkingbrake, clutch=0, gear)` on the player vehicle. Gear is int only (`0` N on hold, `>=1` only with throttle>0). Never `gear=-1`. A failed `set_shift_mode` is logged and retried. With throttle>0 and the echoed gear still `N`/`P`/`R`, a vehicle-Lua arm shifts up from Neutral or jumps to the `D` letter in `automaticModes` (never `shiftDown`, never reverse) and zeros the parking brake. Holds keep `throttle=0` / `brake=1` ±parkingbrake so arcade auto-R cannot fire. Retail: write `Documents/GVD/gvd_cmd.json`; GELua polls it at 20 Hz and applies it as a secondary Direct Drive wheel + pedals (`input.event` source `gvd`, FILTER_DIRECT, `setAllowedInputSource`) — **no DLL**. Retail `applyCmdJson` still zeros parkingbrake (resting-handbrake lockout); this hotfix does not change that bus.
