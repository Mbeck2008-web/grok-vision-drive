...
- **Tech path** (preferred): BeamNGpy Camera sensors, 8-cam rig, BeamNGpy `vehicle.control`. Engaged stop/hold/AEB uses `set_shift_mode("realistic_automatic")` + `gear=0` (neutral) + service brake and, at rest, `parkingbrake` — never `gear=-1`. Drive pins `gear>=1`. Arcade auto-R is not used on Tech. **Retail path**: window-capture fallback for a **single** main view (`cams=1/8`) — do not pretend that is 8 cameras. Retail **drives** through the mod: Python writes `Documents/GVD/gvd_cmd.json`, GELua applies it as a **secondary Direct Drive wheel + pedals** (`input.event(..., FILTER_DIRECT, source=gvd)` plus `setAllowedInputSource` so a connected keyboard/pad/wheel/pedal cluster cannot overwrite the software), and echoes speed/inputs back via `gvd_ego.json`. Retail still zeros `parkingbrake` so a resting handbrake cannot pin the car (unchanged this hotfix).
...
Sim-only. Preferred (Tech): BeamNGpy `set_shift_mode("realistic_automatic")` + `control(...)` on the player vehicle (`get_current` / `get_player_vehicle_id`). Holds: `gear=0` + brake ±parkingbrake. Drive: `gear>=1`. Never `gear=-1`. Retail: write `Documents/GVD/gvd_cmd.json`; GELua polls it at 20 Hz and applies it as a secondary Direct Drive wheel + pedals (`input.event` source `gvd`, FILTER_DIRECT, `setAllowedInputSource`) — **no DLL**. Retail `applyCmdJson` still zeros parkingbrake (resting-handbrake lockout); this hotfix does not change that bus.
