...
def check_player_device_absolute() -> None:
    ...
    for _ in range(8):
        v = sim.step(cmd=(0.4, 0.3, 0.0), echo=(0.0, 0.0, 0.0), player_device=True)
        assert not v.active and abs(v.steer_raw) < 1e-6, f"centered wheel vs GVD 0.4 read as driver: {v}"
