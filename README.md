...
5. Drive onto a road with visible lane lines, then press **Alt+A** or the app's **Engage**. The ribbon appears and the HUD strip reads `modular|ON`; as soon as perception sees both lane lines (`path_debug_preview=false`) GVD takes the wheel and the strip switches to `modular|DRIVE`. Until then it holds the brake (`preview_blocked`). Take over any time by steering it away, touching a pedal, or pressing Alt+A (`driver_override`, sticky until the next Alt+A).

**Force-feedback wheels.** A wheel is welcome: steering override detection carries a deadband, so self-aligning torque, spring centering and kicks over bumps no longer read as you grabbing the wheel and disengage GVD mid-drive — a steer past ~65 % of lock held for ~0.12 s in one direction does, and so does any real brake or throttle press. Tune it in `config\control.yaml` under `override:` if your wheel is noisier or you want a hair trigger.
...
- you steer against it or use a pedal (`driver_override`, sticky until the next Alt+A) — judged by the mod **and** the supervisor, whichever sees it first;
...
**Force-feedback override deadband (this PR):** wheel chatter no longer disengages GVD. Steer override detection runs `residual → deadband → hysteresis → dwell` in a shared `python/control/override.py` and in `gvd_main`, against the envelope of commands GVD actually applied rather than the bare `steering_input` M6 thresholded; pedals stay hard (a real brake / throttle press is an override on the tick it lands, which M6 never checked). Thresholds are tunable in `config/control.yaml` (`override:`) and mirrored to the mod through `gvd_state.json`. The mod now judges override itself instead of waiting for a round trip through `gvd_engage.json`. No dependable force-feedback device type in GE Lua, so the deadband is on whenever engaged — documented assumption; live wheel behaviour **UNPROVEN** until the Windows smoke. Details under [Actuation](#actuation-m3).
...
**Driver override (force-feedback deadband).** `python/control/override.py` and `gvd_main` run the same maths on both sides of the bus, on thresholds `config/control.yaml` owns (`override:`) and the supervisor mirrors into `gvd_state.json` as `override_cfg`. The driver signal is the electrics echo minus the envelope of commands GVD actually applied over `cmd_window_s` (0.30 s), so a gate hold riding along as `brake=1` and the echo lagging our own command are not drivers. Steer then passes a **subtractive deadband** (0.10), so the trip point is `steer_enter + steer_deadband` ≈ 0.65 of lock, and it has to hold one direction for `steer_hold_s` (0.12 s) — a sign flip discharges the dwell, which is what tells a driver leaning on the wheel from a force-feedback wheel shaking itself. Hysteresis: charge above `steer_enter` (0.55), discharge below `steer_clear` (0.30), frozen in between. Judgement waits one `cmd_window_s` after engage, so resting on the brake as you press Alt+A does not override you instantly. Pedals get no deadband and no dwell: `brake_enter` 0.08 / `throttle_enter` 0.15 past the command is an override on the tick it is seen. Above `steer_cmd_max` (0.85) the steer channel bows out — the residual saturates against the ±1 input clamp there. BeamNG GE exposes no dependable force-feedback device type, so `probeFfbWheel` is best effort and **unknown keeps the deadband on whenever engaged** (`ffb_assume_wheel: true`); the supervisor never sees devices at all. Magnitudes are conservative first guesses — no research pin for FFB chatter yet — and live wheel behaviour is still **UNPROVEN** until the Windows smoke. Offline check: `PYTHONPATH=. python scripts/test_ffb_override.py`.
