...
| File | What | How |
| --- | --- | --- |
| `gvd_cabin_synthetic.png` | GVD VISION clean cabin, chase 3/4 just behind the ego, parked (`engaged=false`, title **OFF**, no ice underglow). Agent boxes are empty solids (LEAD / BRAKE, no center disc). Ground, stub lanes, and curbs span the `cameras.yaml` viz range. The cars are an authored traffic pack, not a live detector. The ice ribbon is the authored smoke path (~60 m), not a live planner and not stretched to the horizon | `python.viz.stage.smoke` (`scripts/render_readme_media.py`) |
